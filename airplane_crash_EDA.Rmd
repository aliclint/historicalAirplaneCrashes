...
top.locations$n <- as.numeric(top.locations$n)
   top.locations %>% mutate(location=reorder(location,n)) %>% ggplot(aes(location, n)) + geom_col(aes(fill=as.factor(n))) + coord_flip() +
     ggtitle("Number of plane crashes by country") + xlab("Country") + ylab("Frequency")+ theme(legend.position = "none")  + scale_fill_brewer(palette='Spectral',direction=-1) +
...
ggplot(top.map.locations, aes( x = long, y = lat)) +
  geom_polygon(aes(fill = n))

top.map.locations %>% ggplot(aes(long,lat)) + geom_polygon(aes(group=group,fill=as.factor(n))) +
  labs(fill = 'Countries '
